...
class Database:
    ...
    def add_otp_record(cls, otp_data: Dict):
        try:
            ...
            if country not in data["statistics"]:
                data["statistics"][country] = 0
            data["statistics"][country] += 1
    ...
    def cleanup_otps(cls):
        ...
        last_cleanup = datetime.fromisoformat(data.get("last_cleanup", datetime.now(timezone.utc).isoformat()))
        now = datetime.now(timezone.utc)
        
        if (now - last_cleanup).total_seconds() >= 12 * 3600 or len(data.get("otps", [])) > 500:
            otps = data.get("otps", [])
            if len(otps) > 2:
                data["otps"] = otps[-2:]
    ...
    def get_statistics(cls) -> Dict:
        ...
        total_otps = sum(stats.values()) if isinstance(stats, dict) else 0
        total_numbers = cls.get_total_numbers()
        total_countries = len(stats) if isinstance(stats, dict) else 0
    ...
    @classmethod
    def get_traffic_by_service(cls) -> Dict:
        data = cls.load_db("otps")
        ...
        for otp in otps_list:
            ...
            service = otp.get("service", "Unknown").lower()
            country = otp.get("country", "Unknown")
            
            if service not in service_stats:
                service_stats[service] = {}
            
            if country not in service_stats[service]:
                service_stats[service][country] = 0
            
            service_stats[service][country] += 1
...
class Utils:
    ...
    def extract_numbers_from_file(file_content: str) -> List[str]:
        ...
        for line in file_content.split('\n'):
            ...
            phone_matches = re.findall(r'\b\d{11,15}\b', line)
...
class OTPReceiver:
    ...
    <blockquote>» 𝗗𝗲𝘁𝗮𝗶𝗹𝘀 : {flag} <b>{country}</b> — {service}</blockquote>
    <blockquote>» 𝗡𝘂𝗺𝗯𝗲𝗿 : <code>+{phone}</code></blockquote>
    <blockquote>» 𝗠𝗲𝘀𝘀𝗮𝗴𝗲:</blockquote>
    ...
    """
                        ...
                        # PERBAIKAN: Hapus disable_web_page_preview karena sudah ada link_preview_options
...
<blockquote>© 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿 <a href="{self.config.OWNER_LINK}">𝑲𝒂𝒏𝒈𝑫𝒂𝒚𝒁亗</a> </blockquote>"""
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton("📊 𝚃𝚁𝙰𝙵𝙵𝙸𝙲", callback_data="traffic_menu")
            ]
        ]
...
"""
        ...
        for country, info in country_counts.items():
            message += f"<b>{info['flag']} {country}</b>\n"
        
        message += "\n<blockquote>» 𝗡𝘂𝗺𝗯𝗲𝗿 𝗟𝗶𝘀𝘁</blockquote> \n"
        for num in numbers:
    ...
    async def new_number_request_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        caption = (
            "<blockquote>𝗦𝗲𝗻𝗱 𝗬𝗼𝘂𝗿 𝗻𝘂𝗺𝗯𝗲𝗿𝘀 𝗳𝗼𝗿 𝗳𝗮𝘀𝘁 𝗼𝘁𝗽𝘀</blockquote>\n\n"
            "𝙼𝚒𝚗𝚒𝚖𝚊𝚕 𝟷 𝚗𝚞𝚖𝗯𝗲𝗿𝘀 𝚊𝚗𝚍 𝚖𝚊𝚡 𝟷𝟶 𝚗𝚞𝚖𝗯𝗲𝗿𝘀\n"
            "𝙾𝚝𝚙𝚜 𝚜𝚎𝚗𝚝 𝚝𝚘 𝚢𝚘𝚞𝚛 𝚙𝚛𝚒𝚟𝚊𝚝𝚎 𝚌𝚑𝚊𝚝\n\n"
            "<blockquote>©𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗗𝗮𝘆𝘇𝗗𝗶𝗴𝗶𝘁𝗮𝗹 𝗢𝗳𝗳𝗶𝗰𝗶𝗮𝗹亗</blockquote>"
        )
...
»  𝗗𝗲𝘁𝗮𝗶𝗹𝘀 : {flag}{country} = {service_name}
»  𝗖𝗮𝗽𝗮𝗰𝗶𝘁𝘆 : {capacity}
...
"""
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton("🌐 𝙰𝙱𝙾𝚄𝚃", url=self.config.OWNER_LINK)
            ]
        ]
...
"""
        ...
        for service, service_ranges in services.items():
            ...
            for range_data in service_ranges:
                ...
                message += f"<b>ID : {range_id}</b> {flag}{country} {service} {count}\n"
                message += f" ﹂❯ {filename}\n"
        
        keyboard = [
            [
                InlineKeyboardButton("➕ 𝙰𝙳𝙳 𝚁𝙰𝙽𝙶𝙴𝚂", callback_data="new_ranges"),
                InlineKeyboardButton("🗑️ 𝙳𝙴𝙻𝙴𝚃𝙴 𝚁𝙰𝙽𝙶𝙴𝚂", callback_data="delete_ranges")
            ],
            [
...
"""
    ...
    async def groups_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        message = "<blockquote>❯ 𝗔𝗖𝗧𝗜𝗩𝗘 𝗚𝗥𝗢𝗨𝗣𝗦</blockquote>\n\n"
        ...
        keyboard = [
            ...
            InlineKeyboardButton("🗑️ 𝙳𝙴𝙻𝙴𝚃𝙴 𝙶𝚁𝙾𝚄𝙿𝚂", callback_data="delete_groups")],
            [InlineKeyboardButton("⬅️ 𝙱𝙰𝙲𝙺", callback_data="owner_menu")]
        ]
...
<blockquote>📊 𝗦𝗧𝗔𝗧𝗜𝗦𝗧𝗜𝗖𝗦</blockquote>
"""
        
        message += f"📨 Total OTPs: <b>{stats['total_otps']}</b>\n"
        message += f"📱 Total Numbers: <b>{stats['total_numbers']}</b>\n"
        message += f"🌍 Total Countries: <b>{stats['total_countries']}</b>\n"
        message += f"👥 Total Users: <b>{stats['total_users']}</b>\n"
        message += f"✅ Verified Users: <b>{stats['verified_users']}</b>\n\n"
        
        message += "<blockquote>🚀 𝗧𝗥𝗔𝗙𝗙𝗜𝗖 𝗕𝗬 𝗖𝗢𝗨𝗡𝗧𝗥𝗬</blockquote>\n"
        ...
        for country, count in sorted_traffic[:10]:
            message += f"• {country}: <b>{count}</b> OTPs\n"
...
"""
    ...
    async def del_verif_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        message += "﹂ 𝗦𝗲𝗻𝗱 𝘁𝗵𝗲 𝗚𝗿𝗼𝘂𝗽 𝗜𝗗 𝘆𝗼𝘂 𝘄𝗮𝗻𝘁 𝘁𝗼 𝗱𝗲𝗹𝗲𝘁𝗆𝗲:"
    ...
    async def autodelmsg_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if arg == 'off':
            ...
            groups = Database.get_groups()
            for group_id in groups:
# Di dalam loop for group_id in groups:
                try:
                    message = f"""
<blockquote>𝗡𝗢𝗧𝗜𝗙𝗜𝗖𝗔𝗧𝗜𝗢𝗡 𝗙𝗢𝗥 𝗙𝗢𝗥𝗪𝗔𝗥𝗗 📢</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━
<pre>🚀 𝚂𝚃𝙰𝚁𝚃𝙴𝙳 𝚃𝙾 𝚂𝙴𝙽𝙳 𝚂𝙼𝚂 𝚃𝚁𝙰𝙵𝙵𝙸𝙲
🤖 𝙰𝚄𝚃𝙾𝙼𝙰𝚃𝙸𝙲 𝙳𝙴𝙻 𝙾𝙻𝙳 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 : 𝙾𝙵𝙵
❌ 𝙽𝙾 𝙼𝙴𝚂𝚂𝙰𝙶𝙴 𝚆𝙸𝙻𝙻 𝙱𝙴 𝙳𝙴𝙻𝙴𝚃𝙴𝙳</pre>
━━━━━━━━━━━━━━━━━━━━━━━━━
<blockquote>♾️ 𝙰𝙻𝚆𝙰𝚈𝚂 𝙾𝙽 𝙼𝙴𝚂𝚂𝙰𝙶𝙴</blockquote>
<blockquote>𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗗𝗮𝘆𝘇𝗗𝗶𝗴𝗶𝘁𝗮𝗹 𝗢𝗳𝗳𝗶𝗰𝗶𝗮𝗹</blockquote>"""
                    ...
                    sent_msg = await context.bot.send_message(
                        ...
                        text=message,
                        parse_mode=ParseMode.HTML,
                        link_preview_options=LinkPreviewOptions(is_disabled=True)  # GANTI INI
                    )
            ...
            Database.set_autodel_setting(0, new_notif_ids)
            
            # PERBAIKAN: Cek apakah job_queue tersedia
    ...
    <blockquote>𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗗𝗮𝘆𝘇𝗗𝗶𝗴𝗶𝘁𝗮𝗹 𝗢𝗳𝗳𝗶𝗰𝗶𝗮𝗹</blockquote>"""
                ...
                new_notif_ids[group_id] = sent_msg.message_id
                Database.add_bot_message(group_id, sent_msg.message_id)
    ...
    async def callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif data == "traffic_menu":
            await self.traffic_command(update, context)
    ...
    async def traffic_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        service_stats = Database.get_traffic_by_service()
        
        if not service_stats:
            await update.message.reply_text(
                "📊 <b>No traffic data available yet.</b>",
                parse_mode=ParseMode.HTML
            )
            return
        
        message = "<blockquote>❯ 𝗟𝗜𝗦𝗧 𝗔𝗖𝗧𝗜𝗩𝗘 𝗧𝗥𝗔𝗙𝗙𝗜𝗖 𝗔𝗡𝗗 𝗚𝗢𝗢𝗗 𝗔𝗖𝗖𝗘𝗦</blockquote>\n\n"
        
        countries = Database.get_countries()
        
        for service, country_data in service_stats.items():
            service_name = service.capitalize()
            message += f"<b><blockquote>{service_name}</blockquote></b>\n"
            
            for country, count in country_data.items():
                country_info = next((c for c in countries.values() if c['name'] == country), None)
                flag = country_info['flag'] if country_info else "🌐"
                
                emoji = "🔥" if count >= 200 else "✅"
                message += f"<b><i>{flag}{country} {count} SMS {emoji}</i></b>\n"
