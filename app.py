...
class Database:
    ...
    @classmethod
    def get_traffic_by_service(cls, days: int = 1) -> Dict:
        stats = cls.load_db("daily_stats")
        if not isinstance(stats, dict):
            return {}
        ...
        for i in range(days):
            ...
            if date in stats:
                ...
                for service, count in day_stats.get("services", {}).items():
                    ...
                    for country, country_count in day_stats.get("countries", {}).items():
                        if service in day_stats.get("services", {}):
                            if country not in service_stats[service_name]:
                                service_stats[service_name][country] = 0
                            service_stats[service_name][country] += country_count
...
"""
    ...
    async def fastotps_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Different message for owner vs regular users
        if user.id == self.config.OWNER_ID:
            caption = (
    ...
    async def process_fastotps(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Different limit for owner vs regular users
        if user.id == self.config.OWNER_ID:
            numbers = numbers[:200]  # Owner can send up to 200 numbers
            max_limit = 200
        else:
            numbers = numbers[:10]   # Regular users limited to 10 numbers
            max_limit = 10
...
"""
        ...
        # Display only first 20 numbers to avoid message too long
...
📨 All Time OTPs: <b>{stats['total']}</b>
🌍 Total Countries: <b>{len(stats['countries'])}</b>
📱 Total Numbers: <b>{total_numbers}</b>
...
<blockquote>𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗕𝘆 𝗗𝗮𝘆𝘇𝗗𝗶𝗴𝗶𝘁𝗮𝗹 𝗢𝗳𝗳𝗶𝗰𝗶𝗮𝗹</blockquote>"""
    ...
    async def traffic_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user.id == self.config.OWNER_ID and context.args:
            arg = context.args[0].lower()
            
            if arg == "all":
                stats = Database.get_all_time_stats()
                message = "<blockquote>❯ 𝗔𝗟𝗟-𝗧𝗜𝗠𝗘 𝗧𝗥𝗔𝗙𝗙𝗜𝗖</blockquote>\n\n"
                message += f"📊 Total OTPs: <b>{stats['total']}</b>\n\n"
                
                if stats['countries']:
                    message += "<blockquote>🌍 𝗕𝗬 𝗖𝗢𝗨𝗡𝗧𝗥𝗬</blockquote>\n"
                    sorted_countries = sorted(stats['countries'].items(), key=lambda x: x[1], reverse=True)
                    for country, count in sorted_countries[:15]:
                        message += f"• {country}: <b>{count}</b>\n"
                
                if stats['services']:
                    message += "\n<blockquote>📱 𝗕𝗬 𝗦𝗘𝗥𝗩𝗜𝗖𝗘</blockquote>\n"
                    sorted_services = sorted(stats['services'].items(), key=lambda x: x[1], reverse=True)
                    for service, count in sorted_services[:15]:
                        message += f"• {service}: <b>{count}</b> OTPs\n"
                ...
                try:
                    ...
                    stats = Database.get_traffic_for_days(days)
                    message = f"<blockquote>❯ 𝗧𝗥𝗔𝗙𝗙𝗜𝗖 𝗟𝗔𝗦𝗧 {days} 𝗗𝗔𝗬{'S' if days > 1 else ''}</blockquote>\n\n"
                    message += f"📊 Total OTPs: <b>{stats['total']}</b>\n\n"
                    ...
                    if stats['dates']:
                        message += f"\n<blockquote>📅 𝗗𝗔𝗧𝗘𝗦 𝗜𝗡𝗖𝗟𝗨𝗗𝗘𝗗</blockquote>\n"
                        for date in stats['dates'][:7]:
                            message += f"• {date}\n"
                        if len(stats['dates']) > 7:
                            message += f"• ... and {len(stats['dates']) - 7} more days\n"
                
                except ValueError:
                    service_stats = Database.get_traffic_by_service(1)
                    ...
                    for service, country_data in service_stats.items():
                        message += f"<blockquote><b>{service}</b></blockquote>\n"
